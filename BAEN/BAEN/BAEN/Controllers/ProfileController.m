...
- (void)finishedLoading {
    [header setTitle:[(NETUser *) source identifier]];
    [header setSubtitle:[NSString stringWithFormat:@"User for %@.", [(NETUser *) source created]]];
    [tableView reloadData];
}

- (BOOL)hasAbout {
    return [[(NETUser *) source about] length] != 0;
}
...
- (CGSize)aboutSize {
    NETObjectBodyRenderer *renderer = [(NETUser *)source renderer];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([indexPath section] == 0) {
        if ([indexPath row] == 0 && [self hasAbout]) {
            ...
            [textView setRenderer:[(NETUser *)source renderer]];
            ...
            if (row == 0) {
                ...
                [[cell detailTextLabel] setText:[NSString stringWithFormat:@"%d", [(NETUser *) source karma]]];
            } else if (row == 1) {
                [[cell textLabel] setText:@"average"];
                [[cell detailTextLabel] setText:[[NSNumber numberWithFloat:[(NETUser *) source average]] stringValue]];
            }
        }
...
- (void)tableView:(UITableView *)table didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if ([indexPath section] == 0) {
        ...
        if ([indexPath row] == 0) {
            type = kNETEntryListIdentifierUserSubmissions;
            title = @"Submissions";
            controllerClass = [SubmissionListController class];
        } else if ([indexPath row] == 1) {
            type = kNETEntryListIdentifierUserComments;
            title = @"Comments";
            controllerClass = [CommentListController class];
        }
        
        NETEntryList *list = [NETEntryList session:[source session] entryListWithIdentifier:type user:(NETUser *) source];
...
- (NSString *)sourceTitle {
    return [(NETUser *) source identifier];
}

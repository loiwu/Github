...
- (void)setComment:(NETEntry *)comment_ {
    [comment autorelease];
...
+ (BOOL)entryShowsPoints:(NETEntry *)entry {
...
+ (CGFloat)bodyHeightForComment:(NETEntry *)comment withWidth:(CGFloat)width indentationLevel:(NSInteger)indentationLevel {
    width -= ([self margins].left + [self margins].right);
    width -= (indentationLevel * [self indentationDepth]);
    
    NETObjectBodyRenderer *renderer = [comment renderer];

...
typedef enum {
    kNETPageLayoutTypeUnknown,
    kNETPageLayoutTypeEnclosed, // <table> inside <tr>[3]
    kNETPageLayoutTypeHeaderFooter, // <table>[1:2] inside <tr>[3]
    kNETPageLayoutTypeExposed // <tr>[3:]
} NETPageLayoutType;

@implementation NETAPIRequestParser
...
- (NETPageLayoutType)pageLayoutTypeForDocument:(XMLDocument *)document {
    NSArray *elements = [self mainRowsForDocument:document];
    ...
    if ([elements count] >= 4) {
        ...
        if (td != nil) {
            if ([[td children] count] == 0) {
                return kNETPageLayoutTypeExposed;
            } else {
                ...
                if ([tables count] >= 2 || [linebreaks count] == 2) {
                    return kNETPageLayoutTypeHeaderFooter;
                } else if ([tables count] == 1) {
                    return kNETPageLayoutTypeEnclosed;
                }
            }
        }
    }
    
    return kNETPageLayoutTypeUnknown;
}
...
- (XMLElement *)rootElementForDocument:(XMLDocument *)document pageLayoutType:(NETPageLayoutType)type {
    if (type == kNETPageLayoutTypeHeaderFooter) {
        XMLElement *bodyCell = [self bodyCellElementForDocument:document];
...
- (NSArray *)contentRowsForDocument:(XMLDocument *)document pageLayoutType:(NETPageLayoutType)type {
    if (type == kNETPageLayoutTypeEnclosed) {
        XMLElement *bodyCell = [self bodyCellElementForDocument:document];
        NSArray *elements = [bodyCell elementsMatchingPath:@"table/tr"];
        return elements;
	} else if (type == kNETPageLayoutTypeExposed) {
        NSArray *elements = [self mainRowsForDocument:document];
        NSInteger index = [self bodyIndexForDocument:document];
        return [elements subarrayWithRange:NSMakeRange(index, [elements count] - index)];
    } else if (type == kNETPageLayoutTypeHeaderFooter) {
        XMLElement *bodyCell = [self bodyCellElementForDocument:document];
        ...
        if ([[element attributeWithName:@"class"] isEqual:@"title"]) {
            for (XMLElement *element2 in [element children]) {
                if ([[element2 tagName] isEqual:@"a"] && ![[element2 content] isEqual:@"scribd"]) {
                    ...
                    // In "ask NET" posts, we need to extract the id (and fix the URL) here.
        ...
        if ([[element attributeWithName:@"class"] isEqual:@"default"]) {
            ...
            for (XMLElement *element2 in [element children]) {
                if ([[element2 tagName] isEqual:@"img"] && [[element2 attributeWithName:@"src"] hasSuffix:@"s.gif"]) {
                    // Yes, really: NET uses a 1x1 gif to indent comments. It's like 1999 all over again. :(
...
- (NSDictionary *)parseCommentTreeInDocument:(XMLDocument *)document {
    NETPageLayoutType type = [self pageLayoutTypeForDocument:document];
...
- (NSDictionary *)parseWithString:(NSString *)string {
    ...
    NETPageLayoutType pageLayoutType = [self pageLayoutTypeForDocument:document];
    
    if (userLabel != nil && [[userLabel content] hasPrefix:@"user:"]) {
        result = [self parseUserProfileWithString:string];
    } else if (commentSpan != nil || pageLayoutType == kNETPageLayoutTypeHeaderFooter) {

...
- (void)loadView {
    ...
    toolbar = [[ColorfulToolbar alloc] init];
    [toolbar sizeToFit];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad && [toolbar respondsToSelector:@selector(setBarTintColor:)]) {
        // On iOS 7, toolbars are transparent, so showing colorful on top of colorful looks wrong.
        [toolbar setColorful:NO];
    } else {
        [toolbar setColorful:![[NSUserDefaults standardUserDefaults] boolForKey:@"disable-colorful"]];
    }
    
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"disable-colorful"] && ([toolbar respondsToSelector:@selector(setBarTintColor:)] || [[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad)) {

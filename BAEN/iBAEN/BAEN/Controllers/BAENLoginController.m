...
- (void)viewDidLoad {
    ...
    [topLabel setText:@"BAEN User Log In"];
    [bottomLabel setText:@" Your info is only shared with BAEN."];
    
    if ([self respondsToSelector:@selector(topLayoutGuide)]) {
        [topLabel setTextColor:[UIColor blackColor]];
        [bottomLabel setTextColor:[UIColor darkGrayColor]];
        
        [topLabel setFont:[UIFont fontWithName:@"HelveticaNeue-Light" size:25.0]];
    } else {
...
- (NSArray *)gradientColors {
    ...
    [array addObject:(id) [[UIColor lightColorfulColor] CGColor]];
    [array addObject:(id) [[UIColor mainColorfulColor] CGColor]];
    return array;
}

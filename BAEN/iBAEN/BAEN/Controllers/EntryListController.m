...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (![[NSUserDefaults standardUserDefaults] boolForKey:@"disable-colorful"]) {
        [refreshControl setTintColor:[UIColor lightColorfulColor]];
    } else {
...
- (void)loadEntries {
    [entries release];
    entries = [[(NETEntry *) source entries] copy];
}
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)table {
    if ([source isLoaded]) {
        if ([(NETContainer *) source moreToken] != nil) {
...
- (void)loadMorePressed {
    if ([source isLoaded] && ![(NETContainer *) source isLoadingMore]) {
        [(NETContainer *) source beginLoadingMore];

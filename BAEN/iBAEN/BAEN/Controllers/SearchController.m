...
- (void)loadView {
    ...
    coloredView = [[ColorfulToolbar alloc] initWithFrame:[searchBar bounds]];
...
- (NETAPISearch *)searchAPI {
	if (searchAPI == nil) {
		searchAPI = [[NETAPISearch alloc] initWithSession:session];
	}
...
- (void)facetSelected:(id)sender {
	...
	if ([facetControl selectedSegmentIndex] == 0) {
		[searchAPI setSearchType:kNETSearchTypeInteresting];
	} else {
		[searchAPI setSearchType:kNETSearchTypeRecent];
	}
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    NETEntry *entry = [entries objectAtIndex:[indexPath row]];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [coloredView setColorful:![[NSUserDefaults standardUserDefaults] boolForKey:@"disable-colorful"]];
    ...
    if (![[NSUserDefaults standardUserDefaults] boolForKey:@"disable-colorful"]) {
        if ([coloredView respondsToSelector:@selector(setBarTintColor:)]) {
            ...
            [facetControl setTintColor:[UIColor mainColorfulColor]];
        }
        
        [searchBar setTintColor:[UIColor mainColorfulColor]];
    } else {
